...
> 이 절은 스펙 기반 개발 프레임워크가 쓰는 `constitution.md`(모든 후속 산출물의 기준점이 되는 단일 원칙 문서)와 같은 역할을 한다 — 별도 파일을 새로 만들 필요 없이 이미 이 위치가 그 기능을 한다 (근거: [docs/00-control-tower/external-reference-survey.md](../../docs/00-control-tower/external-reference-survey.md#스펙-기반-개발-프레임워크-비교)).
